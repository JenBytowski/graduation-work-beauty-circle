...
export class MastersListComponent implements OnInit {
  ...
  public improveGetMasters(filtre: Filtre) {
    this.masterList
      ...
      .subscribe((data) => {
        data.forEach(
          (item) =>
            ((item as any).starRating = this.countStarRating(
              item.averageRating
            ))
        );
  ...
  public fillFiltre() {
    this.vm.filtre.cityId = (this.cityId as any)?.el.value
      ? (this.cityId as any).el.value.toString()
      : null;
    this.vm.filtre.serviceTypeIds = (this.serviceTypes as any)?.el.value
      ? (this.serviceTypes as any).el.value.toString().split(' ')
      : null;
    this.vm.filtre.startHour = (this.startHour as any)?.el.value
      ? (this.startHour as any).el.value.toString()
      : null;
    this.vm.filtre.endHour = (this.endHour as any)?.el.value
      ? (this.endHour as any).el.value.toString()
      : null;
    this.vm.filtre.skip = (this.skip as any)?.el.value
      ? (this.skip as any).el.value.toString()
      : 0;
    this.vm.filtre.take = (this.take as any)?.el.value
      ? (this.take as any).el.value.toString()
      : 10;
  }
